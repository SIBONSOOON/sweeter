          ...
          let html_temp = `<div class="box" id="${post["_id"]}">
                                            <article class="media">
                                                <div class="media-left">
                                                    ...
                                                    <img id="profile-image" src="/static/${
...
function toggle_like(postId, action) {
  console.log(`Toggle like for post ${postId} with action: ${action}`);
}
...
function toggle_like(post_id, type) {
  let $i_like = $(`#${post_id} i[aria-label='heart']`);
  $.ajax({
    type: "POST",
    url: "/update_like",
    data: {
      ...
      type_give: type,
      action_give: $i_like.hasClass("fas") ? "unlike" : "like",
    },
    success: function (response) {
      window.location.reload();
      if (response["result"] === "success") {
        $i_like.toggleClass("fas far");
        let $like_num = $i_like.closest("a").find("span.like-num");
        $like_num.text(num2str(response["count"]));
      }
    },
  });
}
